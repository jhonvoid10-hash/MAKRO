...
from adb_utils import wait_for_ui, screencap_base64, tap, tap_recorded, load_recorded_touches
...
Untuk setiap tombol yang relevan dengan state, berikan koordinat x,y TENGAH tombol tersebut.
Koordinat harus dalam pixel sesuai resolusi layar yang diberikan.
...
def detect_state(b64: str, screen_w: int = 720, screen_h: int = 1600) -> dict:
    """
...
def _check_play_solo(b64: str) -> tuple:
    result = detect_state(b64)
    state  = result.get("state", "")
    btn    = result.get("play_solo", {})
    if state == "lobby" and btn.get("visible", False):
        return True, result
    # Juga terima jika state lobby tapi koordinat ada (beberapa model tidak set visible)
...
def _check_continue(b64: str) -> tuple:
    ...
    if state == "continue":
        return True, result
    # Fallback: state lain tapi tombol continue visible
...
# ============================================================
#  QUICK STATE CHECK (tanpa kirim ke Claude — hanya screenshot)
# ============================================================
